...
final class LandingPageController
{
    ...
    public function estimationNantes(): void
    {
        UtmTrackingService::capture();

        View::renderBare('landing/layout', [
            'page_title'       => 'Estimation Immobilière Nantes | Gratuite en 60 secondes',
            'meta_description' => 'Obtenez une estimation immobilière gratuite à Nantes en 60 secondes. Résultat instantané basé sur les données réelles du marché de nantes.',
            'landing_view'     => 'landing/pages/estimation-nantes',
            'landing_slug'     => 'estimation-nantes',
        ]);
    }

    public function vendreMaisonNantes(): void
    {
        UtmTrackingService::capture();

        View::renderBare('landing/layout', [
            'page_title'       => 'Vendre sa Maison à Nantes | Estimation Gratuite',
            'meta_description' => 'Vous vendez votre maison à Nantes ? Obtenez une estimation gratuite et découvrez le prix de vente optimal. Sans engagement.',
            'landing_view'     => 'landing/pages/vendre-maison-nantes',
            'landing_slug'     => 'vendre-maison-nantes',
        ]);
    }
    ...
    public function avisValeurGratuit(): void
    {
        ...
        View::renderBare('landing/layout', [
            'page_title'       => 'Avis de Valeur Gratuit Nantes | Sans Engagement',
            'meta_description' => 'Recevez un avis de valeur gratuit pour votre bien immobilier à Nantes. Analyse basée sur le marché actuel. Résultat immédiat.',
    ...
    public function submitLead(): void
    {
        ...
        try {
            ...
            $ville     = trim((string) ($_POST['ville'] ?? 'Nantes'));
            if ($ville === '') {
                $ville = 'Nantes';
            }
            ...
            $landingSlug = trim((string) ($_POST['landing_slug'] ?? 'estimation-nantes'));
            $landingView = 'landing/pages/' . preg_replace('/[^a-z0-9\-]/', '', $landingSlug);

            // Check view exists, fallback to estimation-nantes
            $viewPath = __DIR__ . '/../views/' . $landingView . '.php';
            if (!is_file($viewPath)) {
                $landingView = 'landing/pages/estimation-nantes';
            }

            View::renderBare('landing/layout', [
                'page_title'       => 'Estimation Immobilière Nantes',
                'meta_description' => 'Estimation immobilière gratuite à Nantes.',
