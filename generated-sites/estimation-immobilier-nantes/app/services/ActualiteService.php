...
final class ActualiteService
{
    private const SEARCH_TOPICS = [
        'marché immobilier Nantes actualité prix',
        'immobilier Nantes Métropole tendances',
        'vente immobilière Loire-Atlantique nouveautés',
        'prix immobilier quartiers Nantes évolution',
        'investissement immobilier Nantes CUB',
        'immobilier neuf Nantes programmes',
        'taux crédit immobilier impact Nantes',
        'urbanisme Nantes projets aménagement',
    ];

    /**
     * Search Perplexity for real estate news around Nantes.
     */
    public function searchNews(?string $customQuery = null): array
    {
        ...
        $prompt = sprintf(
            "Recherche les actualités immobilières récentes à Nantes et ses alentours (Loire-Atlantique, Pays de la Loire) sur le thème : \"%s\".\n"
        ...
        $response = $this->postJson($endpoint, [
            ...
            'temperature' => 0.3,
            'messages' => [
                ['role' => 'system', 'content' => 'Tu es un expert en veille immobilière sur Nantes et sa métropole. Tu fournis des actualités factuelles et récentes.'],
    ...
    public function generateArticleFromIdeas(array $ideas, string $query): array
    {
        ...
        if ($apiKey === '' || empty($ideas)) {
            return $this->fallbackArticle($ideas[0] ?? ['title' => 'Actualité immobilière Nantes']);
        }
        ...
        $prompt = "Tu es un rédacteur expert en immobilier à Nantes. Voici 5 idées d'articles d'actualité immobilière :\n\n"
            . $ideasText . "\n"
            . "1. Choisis la MEILLEURE idée (la plus intéressante, actuelle et utile pour des propriétaires/vendeurs de nantes).\n"
        ...
        $response = $this->postJson($endpoint, [
            ...
            'messages' => [
                ['role' => 'system', 'content' => 'Tu es un journaliste immobilier spécialisé sur Nantes et la Loire-Atlantique. Tu rédiges des articles professionnels et engageants.'],
                ['role' => 'user', 'content' => $prompt],
            ],
        ], [
            'Authorization: Bearer ' . $apiKey,
    ...
    {
        if (trim($imagePrompt) === '') {
            $imagePrompt = 'Professional real estate photography of beautiful Nantes architecture, stone buildings, sunny day, editorial style';
        }
    ...
    {
        return [
            ['title' => 'Évolution des prix immobiliers à Nantes ce mois', 'summary' => 'Les prix au m² continuent leur ajustement dans les quartiers centraux.', 'angle' => 'Analyse quartier par quartier'],
            ['title' => 'Nouveaux projets urbains en métropole de nantese', 'summary' => 'Plusieurs projets d\'aménagement transforment le paysage immobilier.', 'angle' => 'Impact sur les valeurs immobilières'],
            ['title' => 'Taux de crédit : impact sur le marché de nantes', 'summary' => 'L\'évolution des taux influence les décisions d\'achat et de vente.', 'angle' => 'Opportunités pour vendeurs'],
            ['title' => 'Le marché locatif étudiant à Nantes', 'summary' => 'La demande locative étudiante reste forte dans certains quartiers.', 'angle' => 'Investissement locatif'],
            ['title' => 'Rénovation énergétique : les aides disponibles en Loire-Atlantique', 'summary' => 'Les nouvelles réglementations impactent la valeur des biens.', 'angle' => 'Valorisation du patrimoine'],
        ];
    }

    private function fallbackArticle(array $idea): array
    {
        $title = $idea['title'] ?? 'Actualité immobilière Nantes';
        $summary = $idea['summary'] ?? 'Les dernières nouvelles du marché immobilier de nantes.';

        return [
            'title' => $title,
            'meta_title' => $title . ' | Actualités Immobilier Nantes',
            ...
            'content' => '<h2>' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</h2>'
                ...
                . '<p>Le marché immobilier de nantes continue d\'évoluer. Que vous soyez propriétaire souhaitant vendre ou simplement curieux de la valeur de votre bien, il est important de rester informé des dernières tendances.</p>'
                ...
                . '<p>Vous souhaitez connaître la valeur actuelle de votre bien immobilier à Nantes ? '
                . '<strong><a href="/estimation">Lancez votre estimation gratuite</a></strong> et obtenez un résultat en moins de 2 minutes.</p>',
            'image_prompt' => 'Professional editorial photo of Nantes city skyline with stone architecture and Garonne river, warm lighting, real estate magazine style',
        ];
    }
